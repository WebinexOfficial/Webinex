...
const UIUXDesign = () => {
  ...
  useEffect(() => {
    const fetchUIUXProjects = async () => {
      try {
        const response = await fetch('http://localhost:5000/uiProjects'); // 🔁 Replace with your actual API endpoint
        const data = await response.json();
        setUiuxProjects(data);
      } catch (error) {
        console.error('Failed to fetch UI/UX projects:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      {/* Under Development Card (show only if there are no projects) */}
